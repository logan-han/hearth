...
export const WATCHERS: Record<WatcherKind, Watcher> = {
  morning: {
    ...
    instruction: [
      ...
      'First what is on today, with its time and place as given. Then what in the mail the household would act on: anything with a date, a deadline, a payment or a delivery in it (appointments, notices, bills, bookings); leave out newsletters and promotions.',
 ...
 * several people's mail into one room, so each item says whose it was.
 */
export function watcherInstruction(kind: WatcherKind, chatId: string): string {
  const base = WATCHERS[kind].instruction
  return kind === 'morning' && isGroupChat(chatId) ? `${base} Say whose mailbox each item came from.` : base
}
