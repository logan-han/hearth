...
/**
 * Everything the model is told before a word of the conversation: who the
 * family is, what is known, what is still open. Exported because an MCP client
 * gets none of it for free and has to ask; see lib/mcp.ts.
 */
export async function ambientContext(chatId: string, member: Member | null, mode: AgentMode): Promise<Ambient> {
  if (mode === 'watcher') {
  ...
  if (mode === 'sweep') {
    const [memories, questions] = await Promise.all([
      listMemories(SWEEP_MEMORY_LIMIT).catch(() => []),
      openQuestions().catch(() => []),
    ])
    const lines: string[] = []
    if (memories.length) lines.push('Known household facts:', ...memories.map((m) => `- [${m.id}] ${m.content}`))
    // What has already been put to the family, so a night's uncertainty is one question, not one a night.
    if (questions.length) lines.push('Questions already put to the family (do not ask again):', ...questions.map((qn) => `- ${qn.question}`))
    return { text: lines.join('\n'), pendingDrafts: false }
  }

  const [memories, questions, members, connections, drafts, proposals, summary] = await Promise.all([
  ...
  if (questions.length) {
    ...
    lines.push('Questions the nightly pass put to the family, still open (settle with answer_question and the id: pass the fact in their words for a yes or a correction, nothing for a no):')
    lines.push(...questions.map((qn) => `- question_id ${qn.id}: ${qn.question} (would keep: "${qn.candidate}")`))
  }
  if (memories.length) {
...
function chatPrompt(input: { chatType: string; memberName: string }, tz: string): string[] {
  ...
  return [
    ...
    'FILES: a photo, PDF, voice note or file can be read only in the message it arrives with. A calendar file (.ics) arrives as a listing, and import_calendar_file puts its events on the family calendar in one call. If someone refers to a file from an earlier message, ask them to send it again with the request as its caption. A file attached to an email is different: read_email lists its attachments, and read_attachment reads one by the email id and its filename.',
    'FROM MAIL: a ticket, event, reminder or answer made from an email is made after read_email, and after read_attachment when the email says the details are attached: the amount, due date, reference and address come from the email and its attachments, never from the snippet alone.',
    ...
    'MEMORY: Known household facts are already in your context, so use them without announcing them. Call remember when someone asks you to remember something, or corrects a Known fact (pass the old id as replaces). Do not file facts on your own initiative; a nightly pass does that. When someone answers one of the open questions in your context, settle it with answer_question, with the fact in their words for a yes and nothing for a no.',
...
function watcherPrompt(tz: string): string[] {
  return [
    ...
    'POST: for a routine check, one to three short lines a housemate would find useful, the key figure in **bold**. When the instruction asks for a summary in several parts, open with a **bold** title line, put the figures in a two-column pipe table (a label, then its figure), and give each other part a **bold** heading line with its bullets or its table under it. Names, amounts and dates exactly as given.',
    'PURPOSE: say what a payment or message is for only when DATA, a Known fact, a calendar event or a fetched email names it, and say which. Where the instruction asks for a purpose on every line, write "purpose not recorded" for one nothing names; where it does not, say nothing about purpose.',
    ...
    `FORMAT: Telegram markdown: **bold**, *italic*, \`code\`, - bullets, a pipe table for figures (two short columns: a header row, then | --- | --- |, then a row per item; it renders as aligned monospace lines, so keep every cell short), and > at the start of each line of detail worth folding away (a long list of items goes in a > block). No headings, no preamble, no handover line, no commentary about tools. Write in ${language()} with ${units()} units, times in ${tz}.`,
    '',
    'EXAMPLES, where <angle brackets> stand for the real values:',
...
function sweepPrompt(): string[] {
  return [
    ...
    'SOURCES: only a line a family member wrote can become a fact. Lines marked "you" are the assistant\'s own words, among them scheduled posts that summarise mail, notices and bank feeds; read them to make sense of the replies, and never file anything from them: what a "you" line reveals can only become a question, through unsure.',
    'DECIDE for each durable fact you find, in this order. Already Known, or listed as already put to the family: nothing. Stated by a family member, about themselves or a named person: remember. Read in a "you" line, a notice, bill, tuition, booking, newsletter or email, or not saying who it is about: call unsure with the question to put to the family and the fact you would file on a yes, and never remember. A place of study, a club or an activity that turns up in a parent\'s mail is a child\'s, not the parent\'s, and which child is the question.',
    'FILE with remember: people (family, friends, neighbours, and anyone the household deals with regularly), birthdays and anniversaries, health, allergies and dietary needs, routines and standing arrangements, where each person works, studies or trains, clubs and activities, pets, vehicles, sizes, codes and account identifiers, house rules, strong preferences and dislikes.',
    'CORRECT: where a family member contradicts a Known fact, call remember with the new fact and replaces set to the old id. The newer statement wins.',
    'LEAVE OUT one-off events and everything about them (venues, addresses, gates, times, prices); contact details, phone numbers and reporting procedures from notices; bills, due dates and who will pay them; shopping and list items; tasks; passing chatter. The calendar and its proposals hold events.',
    'WRITE each fact self-contained, so it makes sense months from now, and never with a "Source:" note.',
    'EXAMPLE, where <angle brackets> stand for the real values. A "you" line says "<Parent>\'s mail: <place of study> tuition is due <date>" and <Parent> replies "will pay it this week". The one right call is unsure with the question "Who attends <place of study>? A tuition notice was in <Parent>\'s mail." and the fact "A child of the household attends <place of study>". Wrong: remember("<Parent> attends <place of study>"), or filing the due date or the payment.',
 ...
 * over with "Now the post:" — which is how "<payee> is a known budget
 * airline ... Now the post:" once reached the family chat. The instructions already
