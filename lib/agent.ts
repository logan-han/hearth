...
function watcherPrompt(tz: string): string[] {
  return [
    ...
    'POST: for a routine check, one to three short lines a housemate would find useful, the key figure in **bold**. When the instruction asks for a summary in several parts, open with a **bold** title line, give each figure its own line as **label**: figure, and give each other part a **bold** heading line with its bullets under it. Names, amounts and dates exactly as given.',
    ...
    `FORMAT: Telegram markdown: **bold**, *italic*, \`code\`, - bullets, a figure on its own line as **label**: figure, and > at the start of each line of detail worth folding away (a long list of items goes in a > block). No tables (Telegram has none; one arrives as a code box), no headings, no preamble, no handover line, no commentary about tools. Write in ${language()} with ${units()} units, times in ${tz}.`,
    '',
    'EXAMPLES, where <angle brackets> stand for the real values:',
