...
const STUBS = vi.hoisted(() => ({
  list_transactions: () => ({
    account: '2Up',
    transactions: [
      { description: 'FARESAVER LISBON', amount: '$389.60', when: 'Tue 26 Aug 2026, 14:02', status: 'SETTLED', by: 'Rowan' },
      { description: 'FRESHMART 3061 HILLSIDE', amount: '$86.15', when: 'Mon 25 Aug 2026, 18:40', status: 'SETTLED', by: 'Sam' },
    ],
  }),
  ...
  read_email: () => ({
    ...
    body: 'Dear families, the athletics carnival is on Thursday 15 October 2026 from 9am to 12pm at the oval. Students wear house colours.',
  }),
...
describe.skipIf(!liveChainConfigured())('chat grounding', () => {
  it('explains a charge from the feed without inventing a trip', async () => {
    ...
    const text = 'what was that $389 charge on 2up?'
    const r = await runAgent({ chatId: '111', chatType: 'private', member: rowan, memberName: 'Rowan', text })
    const figures = figuresGrounded(r.text, context())
    const hard = figures.ok && noLeak(r.text) && !TRIP_TALK.test(r.text) && /faresaver/i.test(r.text)
    const [g, u] = await Promise.all([judgeGroundedness({ answer: r.text, context: context() }), judgeUsefulness({ task: text, answer: r.text })])
    record({ case: 'chat: $389 charge, no trip', hard: hard ? 'pass' : 'fail', groundedness: g.score, usefulness: u.score, model: r.model, note: r.text.slice(0, 80) })
    expect(figures.missing).toEqual([])
    expect(r.text).not.toMatch(TRIP_TALK)
    expect(r.text).toMatch(/faresaver/i)
  ...
  it('reaches the mail tools from an email cue and reads the date off the notice', async () => {
    const rowan = await q.upsertMember('111', 'Rowan', { allowed: true })
    await q.saveConnection({ memberId: rowan.id, provider: 'google', email: 'rowan@example.com', refreshToken: 'r', scopes: null })
    const text = 'did the school email say when the athletics carnival is?'
    const r = await runAgent({ chatId: '111', chatType: 'private', member: rowan, memberName: 'Rowan', text })
    const usedMail = calls.some((c) => c.tool === 'list_email' || c.tool === 'read_email')
    const hard = usedMail && /15 oct|october 15|15\/10|thursday 15/i.test(r.text) && noLeak(r.text) && !CLOCK_TIME.test(r.text.replace(/9\s?am|12\s?pm|9:00|12:00/gi, ''))
    ...
    expect(r.text).toMatch(/15 oct|october 15|15\/10|thursday 15/i)
    expect(noLeak(r.text)).toBe(true)
  })
  ...
  it('unlocks the mail tools itself when the wording gave no cue', async () => {
    const rowan = await q.upsertMember('111', 'Rowan', { allowed: true })
    await q.saveConnection({ memberId: rowan.id, provider: 'google', email: 'rowan@example.com', refreshToken: 'r', scopes: null })
    const text = 'what did the school say about the athletics carnival?'
    const r = await runAgent({ chatId: '111', chatType: 'private', member: rowan, memberName: 'Rowan', text })
    const unlocked = calls.some((c) => c.tool === 'more_tools' && (c.input as { group?: string }).group === 'mail')
    const usedMail = calls.some((c) => c.tool === 'list_email' || c.tool === 'read_email')
    const hard = unlocked && usedMail && /15 oct|october 15|thursday 15/i.test(r.text) && noLeak(r.text)
    const g = await judgeGroundedness({ answer: r.text, context: context() })
  ...
  it('remembers a fact that scrolled out of the raw window, through the summary', async () => {
    ...
    await q.recordMessage({ chatId: '111', authorName: 'Rowan', role: 'user', content: "Ada's dentist appointment is Thursday 11 Sep at 2pm, Hillside Dental" })
    await q.recordMessage({ chatId: '111', role: 'assistant', content: 'Noted.' })
    for (let i = 0; i < 24; i++) {
      await q.recordMessage({ chatId: '111', authorName: 'Rowan', role: 'user', content: ['what a day', 'traffic was awful', 'did you see the game', 'pasta tonight?', 'ok', 'lol'][i % 6] })
    }
