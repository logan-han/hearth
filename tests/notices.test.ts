...
describe('unsaid', () => {
  ...
  it('keeps a notice about several things until the reply names them all', () => {
    const list = 'Added to the family calendar from school.ics:\n· **Sports day** — Mon, 12 Oct 2026\n· **Assembly** — Tue, 13 Oct 2026, 9:00 am'
    expect(unsaid('Sports day is on the calendar.', [list])).toEqual([list])
    expect(unsaid('Sports day and Assembly are on the calendar.', [list])).toEqual([])
  })
