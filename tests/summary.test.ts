...
async function talk(n: number, from = 1) {
  ...
  for (let i = from; i < from + n; i++) {
    ids.push(await q.recordMessage({ chatId: '-100', authorName: i % 2 ? 'Rowan' : 'Sam', role: 'user', content: `message ${i}` }))
  }
  return ids
}
...
describe('maybeSummarise', () => {
  ...
  it('summarises only what the raw window no longer holds, and remembers how far it got', async () => {
    ...
    expect(stored.summary).toBe('Rowan and Sam talked about messages 1 to 6.')
  ...
  it('puts the summary in front of the chat model, above the household facts', async () => {
    ...
    await runAgent({ chatId: '-100', chatType: 'group', member: null, memberName: 'Rowan', text: "when is Ada's dentist?" })
