...
const post = (body: unknown) =>
  POST(new Request('https://hearth.example/api/admin/telegram', {
...
describe('the Telegram admin API', () => {
  ...
  it('reports an unconfigured bot without calling Telegram', async () => {
    ...
    expect(body.expectedUrl).toBe('https://hearth.example/api/telegram')
  ...
  it('registers the webhook, minting a secret when none is set', async () => {
    ...
    expect(res.status).toBe(200)
    expect(hooked.url).toBe('https://hearth.example/api/telegram')
